...
BOOL SerialComm::GetPortName(TCHAR *dest, UCHAR cbDest)
{
	int cnt;
	if (cbDest < 32) return FALSE;
...
DWORD SerialComm::Receive(TCHAR *buff, INT length, TCHAR *RetMes) {
	if (this->_comHandle == INVALID_HANDLE_VALUE) return -1;
...
DWORD SerialComm::ReceiveAndClose(TCHAR *buff, INT length, TCHAR *RetMes) {
	if (_comHandle == INVALID_HANDLE_VALUE) return -1;

...
unsigned short get_CRC16(unsigned char* DB, unsigned short len, char option)
{
	...
	if (option == CRC16_TEST) {
		polynomial = (CRC16 >> 8)&0x00FF;
		polynomial |= (CRC16 << 8) & 0xFF00;
		return polynomial;
	}
...
unsigned char GetUserCksumIndex(char *srcStr)
{
	unsigned char retValue;	
	// PROTOCOL_DE
	if (srcStr[0] == 'P' && srcStr[1] == 'R' && srcStr[2] == 'O' && srcStr[3] == 'T' &&
		srcStr[9] == 'D' && srcStr[10] == 'E')  retValue = PROTOCOL_DE;
	else retValue = 0x80;	
	return retValue;
}
